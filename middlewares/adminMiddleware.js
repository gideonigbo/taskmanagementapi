...
const adminMiddleware = async (req, res, next) => {
  ...
  try {
    ...
    if (!user || !user.admin) {
      return res.status(401).json({mess: 'Invalid ID / You do not have permission to access this route'})
    }

    req.verifiedUser = user
    if(user.admin) {
      console.log(`Admin: ${user.username} with email: ${user.email} has gained access to protected page`)
    }
    next()
